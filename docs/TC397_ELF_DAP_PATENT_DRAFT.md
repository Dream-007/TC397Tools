...
一种基于 ELF/DWARF 索引文件和 Linux 调试接口的嵌入式目标变量类型化自动读写方法、装置及存储介质
...
- 一种支持 DAP 探针序列号选择和变量类型自动转换的嵌入式目标变量访问方法

## 二、技术领域

本发明涉及嵌入式软件调试、芯片在线调试、自动化测试和可执行文件符号解析技术领域，尤其涉及一种在 Linux 环境下，基于 ELF/DWARF 调试信息生成变量索引文件，并结合硬件调试探针和调试服务接口，实现嵌入式目标芯片变量地址自动解析、末级成员名反查、变量类型自动转换以及变量值自动读写的方法、装置及计算机可读存储介质。
...
8. 多调试探针场景下设备选择不稳定。当同一 Linux 主机连接多个 DAP 或兼容探针时，若仅依赖默认端口序号，容易连接到非目标设备。
9. 原始内存字节与变量语义之间存在转换成本。目标变量可能为有符号整数、无符号整数、单精度浮点或双精度浮点，若脚本层仅返回整型或原始字节，测试逻辑需要重复编写类型转换代码。
10. 批量信号缓存解析不便。目标内存中可能以 `信号 ID、质量因子、数据类型、数据值` 的连续记录形式保存信号快照，现有通用内存读写工具难以直接解析为面向测试的字典结构。

因此，需要一种能够在 Linux 环境中，将 ELF/DWARF 变量解析、结构体字段路径展开、变量索引文件生成、索引一致性校验、硬件调试接口连接、类型化变量读写以及批量信号数据解析进行统一封装的方法，使用户可以通过完整变量路径或末级成员名直接访问目标芯片中的变量值。
...
7. 支持根据变量类型名称和有符号属性，将目标内存字节自动转换为整数、单精度浮点或双精度浮点值，并在写入时执行相反方向的类型化编码；
8. 支持在同一主机连接多个 DAP 探针时，通过 USB 序列号选择目标调试探针；
9. 支持解析由 `sigID、QF、数据类型、数据值` 构成的连续信号数据；
10. 为 Python 等脚本语言提供统一接口，以便自动化测试系统调用。
...
2. 根据用户配置的硬件调试探针序列号，在本机 USB 设备列表中确定目标调试探针对应的调试端口选择号，并通过该硬件调试探针建立与目标嵌入式芯片的调试连接；
3. 加载目标软件对应的 ELF 文件；
...
6. 在变量索引文件中记录 ELF 文件的绝对路径、文件大小、修改时间、摘要值、解析深度和目标软件版本标识；
...
11. 当执行读取时，根据变量信息对象中的类型名称和有符号属性，将读取的原始字节转换为整数、单精度浮点数、双精度浮点数或原始字节；
12. 当执行写入时，根据变量信息对象中的类型名称和访问字节长度，将输入值编码为目标芯片所需的字节序数据；
13. 返回读取结果，或在写入后执行可选的回读验证。
...
1. 初始化调试服务；
2. 枚举本机硬件调试探针 USB 设备，并读取其序列号；
3. 根据用户输入的调试探针序列号确定对应的调试端口选择号；
4. 连接目标芯片；
5. 按目标地址读取指定字节数；
6. 按目标地址写入指定字节数；
7. 断开目标芯片连接。

在一种实施方式中，系统通过 Linux `/sys/bus/usb/devices` 读取 Infineon 或兼容 DAP 设备的厂商 ID、产品信息、总线号、设备号和序列号，并将序列号映射为 DAS/TAS 服务中的端口选择号。用户实例化目标访问对象时输入序列号，连接函数保持无参调用，从而使同一自动化脚本在多调试探针接入场景下仍能稳定连接指定目标设备。
...
在后续读写时，系统不再重复解析 ELF 或 DWARF，而是直接根据变量信息对象中的地址和字节数访问目标芯片。若变量信息对象中的类型名称为 `float` 且访问字节数为 4，则读取结果按照 IEEE 754 单精度浮点数解析；若类型名称为 `double` 且访问字节数为 8，则读取结果按照 IEEE 754 双精度浮点数解析；其他整数类型根据有符号属性和访问字节数解析为有符号或无符号整数。
...
在一种实施方式中，Python 调用 C/C++ 解析模块生成 JSON 变量索引文件时，将目标软件版本标识作为参数传入 C/C++ 模块，由 C/C++ 模块直接写入 JSON 顶层字段。该版本标识仅用于索引文件一致性、追溯和测试报告关联。
...
Python 自动化接口在加载变量索引文件前，读取上述特征信息并与当前 ELF 文件及用户输入的目标软件版本标识进行比较。若 ELF 文件不存在但变量索引文件结构可用，则系统允许在离线场景下继续根据 JSON 索引查询变量地址；若 ELF 文件存在且摘要值、版本标识或其他特征信息不匹配，则系统调用 C/C++ 解析模块重新生成变量索引文件。
...
tas = BaseTas(
    logger=logger,
    dap_serial="AK9D8A6W",
    mcu_version="ZRD.MCU.VERSION.EXAMPLE",
)
...
类型化变量读写模块用于在原始内存字节和脚本层变量值之间进行自动转换。该模块根据变量信息对象中的 `type_name`、`byte_size` 和 `signed` 字段执行如下处理：

1. 当 `type_name` 为 `float` 且 `byte_size` 为 4 时，读取时按小端 IEEE 754 单精度浮点数解析，写入时将输入浮点值编码为 4 字节；
2. 当 `type_name` 为 `double` 且 `byte_size` 为 8 时，读取时按小端 IEEE 754 双精度浮点数解析，写入时将输入浮点值编码为 8 字节；
3. 当变量为整数类型时，根据 `signed` 字段确定有符号或无符号解释方式；
4. 当用户直接输入原始字节或十六进制字节字符串时，系统可跳过类型转换并按原始字节写入；
5. 当访问字节数未知或与变量类型不匹配时，系统返回错误提示或要求用户显式指定字节数。

通过该模块，脚本层接口 `read_variable_value` 可以直接返回整数或浮点数，`write_variable` 可以直接接受整数、浮点数或原始字节，减少测试脚本中重复的 `struct.pack`、`struct.unpack` 和字节序处理。

### 6.10 信号数据记录解析模块

信号数据记录解析模块用于解析从目标内存中读取的一段连续信号数据。每条记录的格式为：

```text
sigID(2 字节) + QF(1 字节) + 数据类型枚举(1 字节) + 数据值(N 字节)
```
...
系统根据数据类型枚举确定数据值的字节长度和解析方式，并将解析结果输出为字典结构，其中键为 `sigID`，值为包含 `QF` 和解析后 `value` 的字典。若解析过程中发现 `sigID` 为 0，则认为后续数据无效并终止解析；若尾部数据不足以构成完整记录，也停止解析并返回已解析结果。该机制适用于目标内存中存在固定长度缓存区且有效数据后跟随填充值或无效尾部数据的场景。
...
8. 提高多设备连接可靠性。通过调试探针序列号选择目标设备，避免多 DAP 接入时因端口顺序变化导致连接错误。
9. 提高数据语义可用性。系统可根据类型名称自动返回整数、单精度浮点或双精度浮点值，并支持对应类型的写入编码。
10. 提高批量信号验证效率。对于连续信号记录缓存，系统可直接解析出以 `sigID` 为键的信号值字典，便于自动化断言。
...
系统首先通过 JSON 变量索引解析变量信息对象，再连接 TAS 调试服务，通过变量信息对象中的地址、访问字节长度和类型信息执行目标内存读写。若目标字段为浮点类型，读取接口直接返回浮点值，写入接口直接接受浮点输入并完成编码。

### 实施例六：连续信号记录解析

当目标内存中存在一段连续信号记录缓存区时，系统通过 TAS 读取该缓存区原始字节，并调用信号数据记录解析模块。对于如下格式的连续记录：

```text
sigID(2B), QF(1B), type(1B), value(NB), sigID(2B), QF(1B), type(1B), value(NB), ...
```
...
```python
{
    0x1234: {"QF": 7, "value": 258},
    0x0003: {"QF": 2, "value": 1.5},
}
```

若解析到 `sigID` 为 0，系统认为后续为无效尾部数据并停止解析，从而避免固定长度缓存区后半段填充值影响有效信号结果。
...
根据权利要求12所述的方法，其特征在于，所述脚本语言接口接收硬件调试探针序列号，根据所述序列号在本机 USB 设备列表中确定对应调试探针，并将其映射为调试服务进程中的端口选择号。

### 权利要求14

根据权利要求1所述的方法，其特征在于，所述读取请求返回的原始字节根据变量信息对象中的变量类型名称、访问字节长度和有符号属性被转换为有符号整数、无符号整数、单精度浮点数或双精度浮点数。

### 权利要求15

根据权利要求1所述的方法，其特征在于，所述写入请求接收整数、浮点数或字节数据，并根据变量信息对象中的变量类型名称和访问字节长度，将所述整数、浮点数或字节数据编码为目标芯片所需的字节序数据。

### 权利要求16

根据权利要求1所述的方法，其特征在于，所述方法还包括解析目标内存中的连续信号数据记录，所述连续信号数据记录包括信号 ID、质量因子、数据类型枚举和数据值，所述脚本语言接口根据数据类型枚举解析数据值，并输出以信号 ID 为键的字典结构。

### 权利要求17

根据权利要求16所述的方法，其特征在于，当所述连续信号数据记录中的信号 ID 为 0，或尾部数据不足以构成完整记录时，停止解析并返回已解析结果。

### 权利要求18

根据权利要求1所述的方法，其特征在于，所述脚本语言接口将目标软件版本标识作为参数传入 C 或 C++ 解析模块，所述 C 或 C++ 解析模块在生成变量索引文件时将所述目标软件版本标识写入所述变量索引文件。
...
类型化读写模块，用于根据变量类型名称、有符号属性和访问字节长度在原始字节与整数或浮点数之间进行转换；

调试探针选择模块，用于根据硬件调试探针序列号选择目标调试探针对应的调试端口；

信号数据解析模块，用于解析包括信号 ID、质量因子、数据类型枚举和数据值的连续信号数据记录；
...
一种计算机程序产品，其特征在于，包括计算机程序，所述计算机程序被处理器执行时实现权利要求1至19任一项所述的方法。

## 十一、摘要初稿

本发明公开了一种基于 ELF/DWARF 索引文件和 Linux 调试接口的嵌入式目标变量类型化自动读写方法、装置及存储介质。该方法通过 C/C++ 解析模块解析目标程序 ELF 文件中的 DWARF 调试信息，生成包括完整变量路径、末级成员名索引、目标地址、访问字节长度、变量类型和目标软件版本标识的 JSON 变量索引文件，并在该索引文件中保存 ELF 文件摘要值等一致性校验信息；Python 自动化接口加载所述变量索引文件，根据完整变量路径或末级成员名生成变量信息对象；随后通过 TAS/DAS 等调试服务接口和指定序列号的硬件调试探针读取或写入目标芯片内存，并根据变量类型自动完成整数、单精度浮点和双精度浮点转换。本发明还支持连续信号记录解析、结构体多级字段路径、末级成员名反查和变量索引一致性校验，能够减少大型 ELF 文件重复解析开销，适用于嵌入式软件调试、自动化测试和目标变量在线访问场景。
...
7. 是否需要将“调试探针序列号选择”“类型化读写”“连续信号记录解析”分别作为从属权利要求保护；
8. 是否需要准备性能对比数据，例如 Python 全量解析耗时、C++ 首次解析耗时、缓存后解析耗时。
